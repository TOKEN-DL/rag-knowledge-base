...
class MCPCitation(BaseModel):
    """问答引用快照。
    与 SSE / 历史接口的 citation 形状对齐，但只保留外部
    Agent 真正需要的字段，避免把 retrieval_meta / rerank_score 等内部调试
    元数据塞给 Agent。
    """
    ordinal: int = Field(description="prompt 中给LLM的「片段 N」编号，从 1 开始")
    document_id: UUID
    document_name: str
...
class MCPUploadResult(BaseModel):
    ...
    version: int
    file_hash: str = Field(
        description="sha256;文件级幂等键，相同 hash复用现有文档",
    )
...
class MCPStats(BaseModel):
    ...
    last_indexed_at: datetime | None = Field(
        default=None,
        description="最近一次进入 ready 状态的文档时间；库为空时返回null",
    )
