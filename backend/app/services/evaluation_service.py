...
from app.evaluation.dataset import list_datasets, EvaluationCase
...
class EvaluationService:
    ...
    async def get_run(self, run_id: UUID) -> EvaluationRun:
        ...
        if run is None:
            raise NotFoundError(f"评测run不存在")
        return run
    ...
    async def get_item(self, item_id: UUID) -> EvaluationItem:
        ...
        if item is None:
            raise NotFoundError("测评 case 不存在")
        return item
...
async def execute_evaluation_run(run_id: UUID) -> None:
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            if run is None:
                logger.warning("evalution run not found, skip: %s", run_id)
                return
        ...
        # 阶段1：逐条跑RAG，每条独立实物写一行item（前端能轮训到进度）
        for case in cases:
            await _run_single_case(run_id, case) # F: 单独运行一条case

        # 阶段2：批量算RAGAS + Bad Case 归因 + 聚合
        await _finalize_run(run_id)   # F: 跑完所有 case 后：一次性算 RAGAS → Bad Case 归因 → 聚合指标。
        ...
        async with AsyncSessionLocal() as session:
            ...
            if run is not None:
                ...
                await session.commit()
        logger.info("evaluation run finished, run_id=%s", run_id)
...
async def _run_single_case(run_id: UUID, case) -> None:
    ...
    async with AsyncSessionLocal() as session:
        ...
        item = EvaluationItem(
            ...
            retrieved_chunks_meta=[_chunk_meta(c) for c in answer.chunks], # F: 提取chunk中的meta
            query_route=answer.query_route or None,
            agent_steps=answer.agent_steps,
            verify_result=_verify_payload(answer.verify_result), #F 提取确认信息和原因：
