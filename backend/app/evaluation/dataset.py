    ...
    with path.open("r", encoding="utf-8") as f:
        for line_no, raw in enumerate(f, start=1):
            ...
            try:
                ...
                raise ValidationError(f"评测集 {name}第{line_no}行JSON非法：{exc.msg}") from exc
            cases.append(_parse_case(data, line_no=line_no, dataset=name))     # 辅助函数 解析数据集
    return cases
...
def _parse_case(data: dict,* , line_no: int, dataset: str) -> EvaluationCase:
    try:
        return EvaluationCase(
            ...
            expected_document_names=data.get("expected_document_names"or []),
