...
class QueryRewriter:
    ...
    async def decide_route(self, question: str) -> QueryRoute:
        ...
        logger.warning("query route模型返回非法值，降级 original： raw=%r", raw)
        return "original"
    ...
    async def apply_route(self, question: str, route: QueryRoute, multi_query_count: int) -> QueryRouterResult:
        ...
        try:
            ...
            if route == "multi_query":
                ...
                return QueryRouterResult(route="multi_query",query=question, multi_queries=queries)
            return QueryRouterResult(route="original", query=question)
        except Exception:
            logger.exception("query_route失败，降级到original:route=%s,question=%r",route, question)
            return QueryRouterResult(route="original", query=question)
    ...
    async def contextualize(self, question: str, history: list[Message]) -> str:
        ...
        """
        history_text = _format_history_text(history) # 辅助函数： 格式化历史文本
        if not history_text:
            return question
        try:
            message = build_contextualize_messages(
            ...
            logger.exception("contextualize调用失败，降级回原问题:question=%r", question)
            return question
