...
def format_content(chunks: list[RetrievedChunk]) -> str:
...
def history_to_messages(history: list[Message]) -> list[BaseMessage]:
    """把数据库Message转成 langchain的BaseMessage，用于塞进prompt。"""
...
def build_answer_messages(
    ...
    """组装最终送个LLM的Messages列表"""
    prompt_value = RAG_ANSWER_PROMPT.invoke(
        {
            "context": format_content(chunks),
            "question": question,
...
def build_multi_query_messages(question: str) -> list[BaseMessage]:
    return list(MULTI_QUERY_PROMPT.invoke({"question": question}).to_messages())
