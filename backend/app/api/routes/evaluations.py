...
@router.post(
    ...
    summary="创建评测run并通过 BackgroundTasks 异步执行",
)
...
@router.get(
    ...
    summary="分页李处run下的case，支持仅看Bad Case与按归因筛选",
)
async def list_evaluation_items(
    ...
    items, total = await service.list_items(
        run_id, page=page, page_size=page_size, bad_case_only=bad_case_only, category=category
    )
